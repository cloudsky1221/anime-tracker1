...
function reducer(state, action) {
  ...
  switch(action.type) {
    case "addItem" : 
      return {
        dataArray:[...state.dataArray, {rank:newNumber, title:"", dots:true, editMode:false}],
        rankLimit:newNumber,
      }

    case "deleteItem" : 
      return {
        dataArray:state.dataArray.filter(e => e.rank !== action.id).map(e => {
...
function Ranking() {
  ...
  const initialValue = JSON.parse(localStorage.getItem("rankings")) || [{rank:1, title:"", dots: true, editMode:false}]

  const [state, dispatch] = useReducer(reducer, {dataArray:initialValue, rankLimit:1})
  ...
  return (
    <>
        ...
        {state.dataArray.map((e,i) => {
          return (
                <li key={i}>
                    ...
                    {e.dots && <button className="buttons" aria-roledescription="show edit and delete button" onClick={() => showMore(e?.rank)}>
                    ...
                    {e.editMode && 
                    <div className="edits">
                      <EditBox id={e?.rank} dispatch={dispatch} setClicks={setClicks}/>
                    </div>}
                </li>
            )
          })}
      ...
      <br />
      {state.rankLimit < 10 && <button className="add" aria-roledescription="Add more to rankings" onClick={handleAdd}>Add</button>}
    </>
  )
}
